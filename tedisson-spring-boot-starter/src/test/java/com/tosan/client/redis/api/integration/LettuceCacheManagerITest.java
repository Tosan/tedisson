...
public class LettuceCacheManagerITest extends BaseITest {
    ...
    @Test
    void testRemoveItemFromHash() {
        String cacheName = getCacheName();
        cacheManager.createCache(cacheName);
        cacheManager.addItemToHash("key", "value");
        cacheManager.removeItemFromHash("key");
        Assertions.assertNull(cacheManager.getItemFromHash("key"));
    }
    ...
    void testUpdateItemExpirationWIthTTL_AND_TTI() throws InterruptedException {
        String cacheName = getCacheName();
        cacheManager.createCache(cacheName);
        cacheManager.addItemToCache(cacheName, "key", "value", 100L, TimeUnit.SECONDS);
        Thread.sleep(1000);
